class PageAppearancesController < ApplicationController
  ...
  def update
    if @page_appearance.update(page_appearance_params)
      redirect_to edit_page_page_appearance_url(@page_appearance), notice: "Page appearance was successfully updated."
    else
  ...
  def set_page_appearance
    @page_appearance = Page.find(params[:page_id]).page_appearance
  end
